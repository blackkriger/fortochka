...
Selective split-tunnel for Windows over **your own** WireGuard connection. Blocked sites and chosen apps go **through** the tunnel; everything else stays on the direct connection.

## What it is

fortochka lives in the system tray and splits into two parts:

- **Engine** — a Windows service (LocalSystem) that runs a userspace WireGuard tunnel, a local SOCKS5/HTTP proxy + PAC, the routing rules, and per-app interception (WinDivert).
...
Packet capture is powered by [WinDivert](https://github.com/basil00/WinDivert).
