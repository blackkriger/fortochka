...
Приложения из Microsoft Store работают внутри движка браузера, а не отдельным процессом, поэтому в списке приложений их нет — а завернуть браузер целиком значит завернуть всё. Их маршрутизируют адресом: например, Instagram в туннель отправляет запись `instagram.com` в списке. 
